...
$(document).ready(function () {
    ...
    $('#programa').on('change', function (event) {

        var sele = $('#FK_Id_Programa').val();
        var text = '<span> Linea </span>' +
            '<select name="FK_Id_Linea" id="FK_Id_Linea">' +
            '<option value="">Seleccionar</option>';
        linea = JSON.parse(linea);
        for (var i = 0; i < linea.length; i++) {
            //console.log(programa[i].FK_Id_Sede + ' ' + parseInt(sele));
            if (linea[i].FK_Id_Programa == parseInt(sele)) {
                text += '<option value="' + linea[i].Id_Linea + '">' + linea[i].Nombre + '</option>';
            }
        }
